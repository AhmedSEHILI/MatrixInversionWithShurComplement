...
double** inverserMatDefPositive(double** matrice, uint64_t n){
    ...
    if ( N == 1){
        ...
        CBinv = multiplicationStrassen(C, Binv, N);
        BinvCT = transposerMatrice(CBinv, N);

        S = soustraireMatrices(D, multiplicationStrassen(C, BinvCT, N), N);
        ...
        SinvCBinv = multiplicationStrassen(Sinv, CBinv, N);

        BinvCTSinv = transposerMatrice(SinvCBinv, N);

        BinvCTSinvCBinv = multiplicationStrassen(BinvCTSinv, CBinv, N);
...
double** inverserMatriceAvecStrassen(double** matrice, uint64_t n){
    ...
    matriceINV = multiplicationStrassen(inverserMatDefPositive(multiplicationStrassen(matriceT, matrice, n), n), matriceT, n);
